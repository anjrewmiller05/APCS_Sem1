...
class starter {
	...
	if (num == 2) {
		System.out.println("What number sentence would you like to play?");
		int sen = sc.nextInt();
		sc.nextLine();
		System.out.println("");
		String sentence = getSentence(sen);
		...
		System.out.println("This was your time: " +time);
		int letters = lettersInSentence(input);
		System.out.println("Letters in sentence: " + letters);
		int words = wordsInSentence(input);
		System.out.println("Words in sentence: " +words);
		double wordsperminute = (60 / time) * words;
		System.out.println("Words per minute: " + wordsperminute);
		int errors = compareSentences(sentence, input);
		System.out.println("Errors : " + errors + "/" + letters);
	}
	...
	if (num == 1) {
		...
		int words = wordsInSentence(input);
		System.out.println("Words in sentence: " +words);
		double wordsperminute = (words/time) * 60;
		System.out.println("Words per minute: " + wordsperminute);
		int errors = compareSentences(sentence, input);
		System.out.println("Errors : " + errors + "/" + letters);
	}
	if (num >= 3) {
		System.out.println("Too bad you don't want to play. It would have been fun!");
	}
